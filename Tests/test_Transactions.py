...
def new_email_transaction(driver):
    ...
    email.delete_emails_from_gmail(MultisigGoogleUser.email, MultisigGoogleUser.imap_code, "Freewallet", "You've received 0.000001 XEM")
    loginPage.login_as_basic_user(ExistingBasicUser.email, ExistingBasicUser.password)
    loginPage.input_pincode_login(ExistingBasicUser.pincode)
    yield
...
class TestClass:
    ...
    def test_send_complex_transaction_to_user_ID(self, driver):
        ...
        transactionsPage.send_transaction_step_4(comment)
        transactionsPage.find_transaction_by_comment("XEM", "0.000001", comment)
    ...
    @pytest.mark.websmoke
    def test_complex_transaction_to_new_address(self, driver):
        comment = str(time.time())
        transactionsPage = TransactionsPage(driver)
        loginPage = LoginPage(driver)
        transactionsPage.navigate_to_send()
        transactionsPage.send_transaction_step_1_user_id("XEM")
        transactionsPage.send_transaction_step_2_user_id(UnregisteredBasicUser.email)
        transactionsPage.send_transaction_step_3("0.000001")
        transactionsPage.send_transaction_step_4(comment)
        password_link = email.get_registration_link_from_email(MultisigGoogleUser.email, MultisigGoogleUser.imap_code, "Freewallet", "You've received 0.000001 XEM")
        ...
        transactionsPage.check_first_transaction_receive("XEM", "0.000001", comment)
